...
export function Particles({
  ...
  const circleParams = (): Circle => {
    const x = Math.floor(Math.random() * (canvasRef.current?.width || 0)) / dpr
    const y = Math.floor(Math.random() * (canvasRef.current?.height || 0)) / dpr
    const translateX = 0
    const translateY = 0
  ...
  const animate = () => {
    ...
    circles.current.forEach((circle: Circle, i: number) => {
      ...
      const edge = [
        ...
        canvasRef.current!.width / dpr - circle.x - circle.translateX - circle.size, // distance from right edge
        circle.y + circle.translateY - circle.size, // distance from top edge
        canvasRef.current!.height / dpr - circle.y - circle.translateY - circle.size, // distance from bottom edge
      ]
      ...
      if (
        ...
        circle.x > canvasRef.current!.width / dpr + circle.size ||
        circle.y < -circle.size ||
        circle.y > canvasRef.current!.height / dpr + circle.size
      ) {
